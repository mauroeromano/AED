...
bool PuntoDentroDePoligono(const Poligono&,const Punto&);
...
int main(){
    ...
    assert(GetCantidadDeLados(p) == 3);
    assert(Top(p).x == 0 and Top(p).y == 1.5);
    assert(PuntoDentroDePoligono(p,{0,1}));
    ...
    assert(PuntoDentroDePoligono(p2,{0,0}));
    ...
    assert(GetCantidadDeLados(p3)==3);
    assert(AreNear(GetPerimetro(p3),10.24));
    assert( PuntoDentroDePoligono(p3,{1,1}));
    assert(!PuntoDentroDePoligono(p3,{9,1}));
}
...
// Indica si un punto esta dentro o no de un Poligono (ray casting)

bool PuntoDentroDePoligono(const Poligono& p,const Punto& punto){
    unsigned n = p.n;
    ...
    for (unsigned i{}; i < n - 1; ++i) {
        x1 = p.puntos.at(i).x;
        x2 = p.puntos.at(i+1).x;
        y1 = p.puntos.at(i).y;
        y2 = p.puntos.at(i+1).y;
